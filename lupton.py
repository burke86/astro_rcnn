...
import numpy as np
from interval import ZScaleInterval
...
class Mapping:
    """
    Baseclass to map red, blue, green intensities into uint16 values.
    ...
    def __init__(self, minimum=None, image=None):
        self._uint16Max = float(np.iinfo(np.uint16).max)
    ...
    def make_rgb_image(self, image_r, image_g, image_b):
        """
        Convert 3 arrays, image_r, image_g, and image_b into an 16-bit RGB image.
        ...
        RGBimage : `~numpy.ndarray`
            RGB (integer, 16-bits per channel) color image as an NxNx3 numpy array.
        """
        ...
        return np.dstack(self._convert_images_to_uint16(image_r, image_g, image_b)).astype(np.uint16)
    ...
    def map_intensity_to_uint16(self, I):
        """
        Return an array which, when multiplied by an image, returns that image
        mapped to the range of a uint16, [0, 255] (but not converted to uint16).
        ...
        with np.errstate(invalid='ignore', divide='ignore'):
            return np.clip(I, 0, self._uint16Max)

    def _convert_images_to_uint16(self, image_r, image_g, image_b):
        """Use the mapping to convert images image_r, image_g, and image_b to a triplet of uint16 images"""
        image_r = image_r - self.minimum[0]  # n.b. makes copy
        image_g = image_g - self.minimum[1]
        image_b = image_b - self.minimum[2]

        fac = self.map_intensity_to_uint16(self.intensity(image_r, image_g, image_b))
            ...
            for i, c in enumerate(image_rgb):
                c = np.where(r0 > g0,
                             ...
                             np.where(g0 > b0,
                                      ...
                                      np.where(b0 >= pixmax, c*pixmax/b0, c))).astype(np.uint16)
                c[c > pixmax] = pixmax
...
class LinearMapping(Mapping):
    """
    A linear map map of red, blue, green intensities into uint16 values.
    ...
    def map_intensity_to_uint16(self, I):
        with np.errstate(invalid='ignore', divide='ignore'):  # n.b. np.where can't and doesn't short-circuit
            return np.where(I <= 0, 0,
                            np.where(I >= self._range, self._uint16Max/I, self._uint16Max/self._range))
...
class AsinhMapping(Mapping):
    ...
    def __init__(self, minimum, stretch, Q=8):
        ...
        self._slope = frac*self._uint16Max/np.arcsinh(frac*Q)
